	...
	<hanzmobview>
		<article class="btn-group col-sm-12">
			<div class="navbar navbar-default" style="background-image: linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% );border-radius: 5px 5px 0px 0px;">
				...
				<a href="<?=base_url()?>master_staff" class="btn btn-default btn-circle btn-sm zoomsmall active">
	...
	<section id="widget-grid" class="">
			...
			<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				data-widget-editbutton="false">
					...
					<header style="background: linear-gradient(to bottom, #00c6ff, #0072ff);">
						...
						<section>
						<a onclick="createModal()" class="btn btn-default btn-xs pull-right zoomsmall" style="float:left; background-image: linear-gradient(120deg, #fdfbfb 0%, #ebedee 100%); border-radius: 4px; margin: 14px 0px 0px 0px;height:28px; width:220px">
						...
						<p class="small" style="margin: 6px 0px 0px 0px; ">
							<small style="color:black;font-size:12px; font-weight: bold;">Tambah Data Karyawan/Staff</small>
						</p>
						</a>
						</section>
						</span>
		...
		<div style="margin: 0px 0px -40px 0px;" class="jarviswidget jarviswidget-color-blueLight" id="wid-id-10" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-togglebutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-custombutton="false" data-widget-sortable="false">
				<header style="background-image: linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% );color:white;">
					<span class="widget-icon"> <img style="float: left; margin: 8px 5px 0px 6px;" src="<?=base_url()?>assets/img/userpro.png" height="18" width="18" /> </span>
					<h2 style="color:white;font-size:14px; font-weight: bold; margin: -2px 0px 0px 10px;">Data Karyawan/Staff
					</h2>
				</header>
						...
						<form action="<?=base_url()?>master_kebutuhan_part/insert" class="formName" style="margin-bottom: 10px">
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<input type="text" placeholder="" class="form-control" id="email2" name="email2" value="" required>
								</div>
							</div>
						</form>
...
<?php $__env->startSection('javascript'); ?>
	...
	<script type="text/javascript">
			...
			*/	
			...
			/* BASIC ;*/
				...
				table = $('#dt_basic').DataTable({
					...
					"ajax":{
						url :  base_url + 'cle_master_staff/get_data',	
						type : 'POST',
			...
			$.confirm({
				...
				buttons: {
					formSubmit: {
						...
						action: function () {
							var self = this;
							var url = '<?=base_url()?>cle_master_staff/insert';
		...
		function createModalEdit(id, nik, nama, alamat, jk, hp, username, password) {
			var content = $('.content_form').clone().html();
			...
			$.confirm({
				...
				buttons: {
					formSubmit: {
						...
						action: function () {
							...
							var url = '<?=base_url()?>cle_master_staff/update';
							var form = self.$content.find('form')[0];
							var formData = new FormData(form);
				...
				onContentReady: function () {
					// bind to events
					var jc = this;
					jc.$content.find('#nik').val(nik);
					jc.$content.find('#nama').val(nama);
					jc.$content.find('#alamat').val(alamat);
					jc.$content.find('#jk').val(jk);
					jc.$content.find('#hp').val(hp);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\DEV_SERVER\htdocs\bridnins\coresys\views/pages/cle_master_staff/index.blade.php ENDPATH**/ ?>