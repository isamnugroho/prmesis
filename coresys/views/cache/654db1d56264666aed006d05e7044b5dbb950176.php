...
<hanzmobview>
	...
	<section id="widget-grid" class="">
				...
				<div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-colorbutton="false" 
					...
					<div>
						...
						<div class="jarviswidget-editbox">
							...
							<a onclick="createModal()" class="btn btn-primary pull-right" style="margin-left: 10px; border-radius: 5px;"><img style="float: left; margin: 0px 5px 0px 0px;" src="<?=base_url()?>seipkon/assets/img/adddata.png" height="20" width="20" />Tambah Data Coverage Detail</a>
							<a href="<?=(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : base_url().'master_lokasi.ins')?>" class="btn btn-warning pull-right" style="border-radius: 5px;"><img style="float: left; margin: 0px 5px 0px 0px;" src="<?=base_url()?>seipkon/assets/img/navigation-180.png" height="20" width="20" />Back</a> 
							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								<thead>			                
									<tr>
										...
										<th data-hide="phone,tablet">Alamat/Lokasi Mesin ATM</th>
		...
		<div class="container content_form" hidden>
			<form action="<?=base_url()?>master_location_detail/insert" class="formName" style="margin-bottom: 10px">
				<div class="form-group">
					<label>Masukan Data Lokasi Mesin ATM</label>
					<input type="hidden" class="form-control" id="id" value="<?=$id?>" required />
					<select class="form-control data_atm" id="atm" required="">
						<option value="">Pilih Lokasi Mesin ATM...</option>
					</select>
				</div>
			</form>
		</div>
			...
			*/	
			...
			/* BASIC ;*/
				...
				table = $('#dt_basic').DataTable({
					...
					"ajax":{
						url :  base_url + 'master_location_detail/get_data/<?=$id?>',
						type : 'POST',
			...
			$.confirm({
				...
				buttons: {
					formSubmit: {
						...
						action: function () {
							...
							var atm = this.$content.find('#atm').val();
							if(!atm){
								$.alert('provide a valid atm');
								return false;
							}
				...
				onContentReady: function () {
					...
					this.$content.find('.data_atm').select2({
						...
						ajax: {
							dataType: 'json',
							url: '<?php echo base_url().'master_location_detail/select_atm'?>',
							delay: 250,
							type: "POST",
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\2021\MAR-W1\atmserv-assindo\coresys\views/pages/master_location_detail/index.blade.php ENDPATH**/ ?>