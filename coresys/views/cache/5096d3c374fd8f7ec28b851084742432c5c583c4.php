	...
	<hanzmobview>
		<article class="btn-group col-sm-12">
			<div class="navbar navbar-default" style="background-image: linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% );border-radius: 5px 5px 0px 0px;">
				...
				<a href="status_ticketflmflm" class="btn btn-default btn-circle btn-sm zoomsmall active">
			...
			<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin: -20px 0px 0px 0px;">
				...
				<div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" 	
					...
					<header style="background: linear-gradient(to bottom, #00c6ff, #0072ff);">
						...
						<h2 style="color:white; margin: -1px 0px 0px 10px;"><b>Data Status Tickets - FLM</b></h2>
		...
		var pagefunction = function() {
			...
			$('.from').datepicker({
				...
				if(selected.date!==undefined) { 
					...
					$.ajax({
						url: '<?=base_url()?>status_ticketflm/show_table',
						dataType: 'html',
						method: 'get',
						data: {id: "", date: date},
						timeout: 3000,
			...
			$.get("<?=base_url()?>status_ticketflm/show_table", {id: "<?=$id?>", date: "<?=$date?>"}, function(response){
				$('#content_table div').remove();
				$('#content_table').html(response);
			});

			function get_data_table() {
				$.get("<?=base_url()?>status_ticketflm/check_data", function(response){
					if(response>0) {
						console.log("UPDATED");
						$.get("<?=base_url()?>status_ticketflm/update_status", function(response){
							if(response=="success") {
								$.get("<?=base_url()?>status_ticketflm/show_table", {id: "<?=$id?>", date: "<?=$date?>"}, function(response){
									$('#content_table div').remove();
									$('#content_table').html(response);
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\DEV_SERVER\htdocs\bridnins\coresys\views/pages/status_ticketflm/index.blade.php ENDPATH**/ ?>