...
<?php $__env->startSection('javascript'); ?>
	<script type="text/javascript">
			...
			/* BASIC ;*/
				function format ( d ) {
					...
					tableZZ += '<tr>'+
						...
						'<th>Rp '+total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")+'</th>'+
					'</tr>';
					tableZZ += '<tr>'+
						'<th colspan="3"></th>'+
			...
			$.confirm({
				...
				buttons: {
					delete: {
						...
						action: function () {
							$.ajax({
								url: "<?=base_url()?>costing_job/costing_approve",
								data: {id: id},
								dataType: 'html',
								timeout: 3000,
		...
		function costing_edit(id) {
			...
			$.get('<?php echo base_url().'costing_job/get_data_costing'?>', {id: id}, function( data ) {
				// console.log();
				var content = '' +
				...
				Object.entries(res).forEach((entry) => {
					...
					// entry[1].biaya = "12.000";
					// console.log(entry[1]);
					
					content += '' +
						'<div class="form-group">' +
							'<label>'+entry[1].pengeluaran+' ('+entry[1].keterangan+')</label>' +
							'<input type="text" placeholder="'+entry[1].biaya+'" value="'+entry[1].cost+'" class="cost_'+entry[0]+' form-control" required />' +
						'</div>' +
						'';
					...
					buttons: {
						delete: {
							...
							action: function () {
								...
								var res = JSON.parse(data);
								Object.entries(res).forEach((entry) => {
									entry[1].biaya = 'Rp '+numberWithCommas(self.$content.find('.cost_'+entry[0]).val());
									entry[1].cost = self.$content.find('.cost_'+entry[0]).val();
								});
								
								// console.log(res);
								$.ajax({
									url: '<?php echo base_url().'costing_job/update_data_costing'?>',
									dataType: 'html',
									method: 'post',
									data: {id: id, detail: JSON.stringify(res)},
									timeout: 3000,
								}).done(function (response) {
									if(response=="success") {
										self.close();
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\xampp\htdocs\2021\JUL-W1\BRIDNINS\coresys\views/pages/costing_job/index.blade.php ENDPATH**/ ?>