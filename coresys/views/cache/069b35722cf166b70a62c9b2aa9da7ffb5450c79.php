...
<?php $__env->startSection('content'); ?>
	...
	<hanzmobview>
		<article class="btn-group col-sm-12">
			<div class="navbar navbar-default" style="background-image: linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% );border-radius: 5px 5px 0px 0px;">
				...
				<div class="col-sm-3" style="margin: 5px 0px 0px 0px;">
				<a href="<?=base_url()?>finance_costing" class="btn btn-default btn-circle btn-sm zoomsmall active">
	...
	<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin: -20px 0px 0px 0px;">
		...
		<div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" 	
			...
			<header style="background: linear-gradient(to bottom, #00c6ff, #0072ff);">
				<h2 style="color:white; margin: -1px 0px 0px 10px;"><b>Data Costing</b></h2>
				
			</header>
			<span class="ribbon-button-alignment pull-right" style="margin: -42px 0px 0px 0px; "> 
				<section>
				<a onclick="createModal2()" class="btn btn-primary" style="background-image: linear-gradient( 171.8deg,  rgba(5,111,146,1) 13.5%, rgba(6,57,84,1) 78.6% );border-radius: 4px;height:30px"><img style="" src="<?=base_url()?>seipkon/assets/img/adddata.png" height="20" width="20" /><b>Tambah Data</b></a>
				</section>
			</span>
					...
					<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
						<thead>			                
							<tr>
								...
								<th data-class="expand">COSTING BIAYA</th>
								<th data-hide="phone,tablet">SERIAL NUMBER</th>
								<th data-hide="phone,tablet">JOB ORDER</th>
								<th data-hide="phone,tablet">BANK</th>
								<th data-hide="phone,tablet">CABANG</th>
	...
	<!-- WIDGET END -->
		...
		<div class="container content_merk" hidden>
			<center>
				<form action="<?=base_url().explode("/", $that->uri->uri_string())[0].'/add_data_temp'?>" style="width: 90%; text-align: left" enctype="multipart/form-data">
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label class="control-label">Costing Biaya :</label>
								<input type="text" placeholder="" class="form-control" id="costing_biaya" name="costing_biaya" value="" required>
							</div>
						</div>
						<div class="col-md-12">
							<div class="form-group">
								...
								<div id="slWrapper" class="parsley-select wd-250">
									<select class="form-control" data-placeholder="Choose one" data-parsley-class-handler="#slWrapper" data-parsley-errors-container="#slErrorContainer" id="serial_number" name="serial_number" required>
										<option value="">Pilih Jenis/Merek Mesin</option>
									</select>
									<div id="slErrorContainer"></div>
								</div>
		...
		<div class="container content_form2" hidden>
			<center>
				<form action="<?=base_url()?>finance_costing/insert" class="formName" enctype="multipart/form-data" style="width: 95%; text-align: left">
					<div class="row">
						<div class="col-md-12">
							<div class="row">
								<div class="col-md-3">
									<div class="form-group" style="margin-bottom: -35px; position: relative; z-index: 1">
										<label class="control-label">Job Order :</label>
										<input type="text" placeholder="" class="form-control" id="job_order" name="job_order" value="" required>
									</div>
								</div>
							</div>
						</div>
						<div class="col-md-12" id="add_detail">
							...
							<table id="datatable2" class="table display table-bordered" >
								<thead>
									...
									<th width="50" style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Costing Biaya</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Serial Number</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Produk</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Bank</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Cabang</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">Lokasi ATM</th>
									<th style="background: linear-gradient(to bottom, #323232 0%, #3F3F3F 0%, #1C1C1C 150%);color:white;font-size:14px;">CSE</th>
...
<?php $__env->startSection('javascript'); ?>
	<script type="text/javascript">
			...
			*/	
			...
			/* BASIC ;*/
				...
				table = $('#dt_basic').DataTable({
					...
					"ajax":{
						url :  base_url + 'costing_rev/tes/0',
						type : 'POST',
				...
				// $("#select_merk").on("change", function() {
					...
					// if(that.val()!=="") {
						// // alert(that.val());
						// table.ajax.url(base_url + 'finance_costing/get_data/'+that.val()).load();
					// } else {
						// table.ajax.url(base_url + 'finance_costing/get_data/0').load();
					// }
				// });
		...
		function createModalMerk() {
			...
			$.ajax({url: '<?=base_url()?>finance_costing/count_data_temp', dataType: 'html', method: 'post', data: { 
			}, timeout: 3000}).done(function (response) {
				var cnt_response = response;
				// alert(qty_global+ " "+response);
				// if(qty_global!==undefined) {
					...
					// } else {
						$.confirm({
							...
							onContentReady: function () {
								...
								jc.$content.find('#serial_number').select2({
									tokenSeparators: [','],
									ajax: {
										dataType: 'json',
										url: '<?php echo base_url().'finance_costing/get_serial'?>',
										delay: 250,
										type: "POST",
									...
									// $.get("<?=base_url()?>finance_costing/get_category", { id: value }, function(res) {
										// alert(res);
										// if(res!="consume") {
		...
		function createModalData() {
			...
			$.ajax({url: '<?=base_url()?>finance_costing/add_data_temp', dataType: 'html', method: 'post', data: { 
			}, timeout: 3000}).done(function (response) {
				table2.ajax.reload( null, false );
			});
		}
		...
		function createModal2() {
			...
			$.confirm({
				...
				buttons: {
					formSubmit: {
						...
						action: function () {
							...
							var job_order = this.$content.find('#job_order').val();
							if(job_order=="") {
								alert("INPUT JOB ORDER FIRST!");
								return false;
							}
					...
					cancel: function () {
						$.ajax({url: '<?=base_url()?>finance_costing/clear_temp', dataType: 'html', method: 'post', data: { 
						}, timeout: 3000}).done(function (response) {
							table2.ajax.reload( null, false );
						});
					},
				...
				onContentReady: function () {
					...
					// $.ajax({url: '<?=base_url()?>finance_costing/get_part', dataType: 'html', method: 'post', data: { merk: $("#select_merk").val() }, timeout: 3000}).done(function (response) {
					...
					table2 = this.$content.find('#datatable2').DataTable({
						...
						"columnDefs": [{"orderable": false, "targets": 0}],
						"ajax":{
							url :  base_url + 'finance_costing/get_data_temp',
							type : 'POST',
							dataFilter: function(data) {
								console.log(data);
								var json = jQuery.parseJSON( data );
		...
		function detailData(id, $cse) {
			console.log(id);
			var base_url = "<?php echo base_url();?>";
			console.log();
			$.confirm({
				title: 'Detail Costing',
				content: 'url:'+base_url + 'costing_rev/detail/'+id,
				onContentReady: function () {
					var self = this;
				...
				buttons: {
					transfer: {
						...
						action: function () {
							$.confirm({
								title: 'Transfer costing?',
								content: 'Menu ini untuk notif ke cse bahwa costing sudah di transfer, \'apakah sudah di transfer?\' <br> Jumlah Transfer : <input id="jumlah_transfer"></input>',
								...
								buttons: {
									delete: {
										...
										keys: ['enter'],
										action: function () {
											var jum = this.$content.find('#jumlah_transfer').val();
											// alert(jum);
											$.ajax({
												url: base_url + 'costing_rev/transfered/'+id+'/'+$cse+'/'+jum,
												dataType: 'html',
											}).done(function (response) {
												alert(response);
												if(response=="success") {
		...
		// table.ajax.url(base_url + 'finance_costing/get_data').load();
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\DEV_SERVER\htdocs\bridnins\coresys\views/pages/costing_rev/index.blade.php ENDPATH**/ ?>