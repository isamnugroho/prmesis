...
class Cle_master_atm extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		$result = $this->db->get("cle_master_atm")->result();
		...
		return view('pages/cle_master_atm/index', $this->data);
	}
	...
	public function ccek() {
		...
		$query = $this->db->query("SELECT * FROM cle_master_atm WHERE id_bank='20' AND idatm NOT IN ('40401', '40601', '40501', '30004', '40201', '30105', '30304', '30011', '30012', '30005', '40103', '30007', '30008', '30006', '40303', '40005', '30103', '40004', '30101', '30315', '30314', '30313', '30201', '300014', '300016', '300015', '30602', '40011', '40013', '40307', '30316', '40012', '30107', '40203', '30501', '12540003', '12530102', '12540102', '12540302', '12530101', '12540002', '12530002', '12530003')")->result();
		$query = $this->db->query("SELECT cle_master_atm.idatm FROM master_location_detail LEFT JOIN cle_master_atm ON (cle_master_atm.id=master_location_detail.id_atm) WHERE master_location_detail.id_lokasi='59'")->result();
	...
	public function resync_by_txt() {
		...
		foreach($atm as $key => $line) {
			if($key==0) {
				...
				$sn_number = $param2[1];	
				$result = $this->db->query("SELECT * FROM cle_master_atm WHERE sn_mesin='".$sn_number."'");
				if($result->num_rows()>0) {
					...
					// $this->db->update('cle_master_atm', $data);
				} else {
					$no++;
					
					$data['id_serial'] = $this->db->query('SELECT id FROM master_serial WHERE id NOT IN (SELECT id_serial FROM cle_master_atm) ORDER BY id ASC LIMIT 0,1')->row()->id;
					$data['id_bank'] = "20";
					$data['idatm'] = $param2[0];
					...
					$result = $this->db->insert('cle_master_atm', $data);
	...
	public function input_by_txt() {
		...
		foreach($atm as $key => $line) {
			...
			if($key==0) {
				...
				$param2 = explode(PHP_EOL, $line);
				$param2 = explode("	", $param2[0]);
				$idatm = $this->db->query("SELECT * FROM cle_master_atm WHERE idatm='".$param2[1]."'");
				...
				if($idatm->num_rows()==0) {
					...
					$query['id_serial'] = $this->db->query('SELECT id FROM master_serial WHERE id NOT IN (SELECT id_serial FROM cle_master_atm) ORDER BY id ASC LIMIT 0,1')->row()->id;
					$this->db->insert('cle_master_atm', $query);
	...
	public function add() {
		...
		return view('pages/cle_master_atm/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("cle_master_atm")->row();
	...
	public function insert() {
		...
		$id_serial = $this->db->query('SELECT id FROM master_serial WHERE id NOT IN (SELECT id_serial FROM cle_master_atm) ORDER BY id ASC LIMIT 0,1')->row()->id;
		...
		if($id==null){
			...
			$data['longitude'] = $longitude;
			$this->db->where('id', $id);
			$result = $this->db->update('cle_master_atm', $data);
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_master_atm');
	...
	public function get_data($id_bank) {
        ...
        $employees = $this->db->get("cle_master_atm");
        $data = array();
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cle_master_atm");
        $result = $query->row();
