...
class Cro_master_client extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cro_master_client/index', $this->data);
	}
	
	public function add() {
		return view('pages/cro_master_client/add');
	}
	
	public function edit() {
		return view('pages/cro_master_client/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('cro_master_client');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('cro_master_client', $data);
				
				if($res) {
					$result['status'] = 'success';
					
					$this->logs_model->insert(array(
						'menu_access' 	=> 'cro_master_client',
			...
			// if($num>0) {
				// $result['status'] = 'exist';
			// } else {
				$data_old = $query = $this->db->get_where('cro_master_client', array('id' => $id), 1, 0)->row();
				$this->db->where('id', $id);
				$res = $this->db->update('cro_master_client', $data);
	...
	public function delete($id) {
		$data_old = $this->db->get_where('cro_master_client', array('id' => $id), 1, 0)->row();
		$this->db->where('id', $id);
        $result = $this->db->delete('cro_master_client');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cro_master_client");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cro_master_client';
			$url2 = base_url().'cro_master_client';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cro_master_client");
        $result = $query->row();
