...
class Cle_report_cleaning extends MY_Controller {
	...
	public function index() {
		...
		$this->db->from('cle_master_atm');
		$this->db->join('cle_master_location_detail', 'cle_master_atm.id=cle_master_location_detail.id_atm');
		$query = $this->db->get();
		$result = $query->result();
		
		$this->data['data_atm'] = $result;
		
		return view('pages/cle_report_cleaning/index', $this->data);
	}
	
	public function index2() {
		echo "<center><a href='".base_url()."cle_report_cleaning/generate/output' target='_blank'>OUTPUT</a></center>";
		echo "<center><a href='".base_url()."cle_report_cleaning/generate/preview' target='_blank'>PREVIEW</a></center>";
	}
	
	public function func() {
		$this->terpakai = function($kode_part) {
			$qty = $this->db->query("SELECT kode_part, SUM(quantity) as qty FROM cle_master_transaction_out WHERE kode_part='$kode_part' GROUP BY kode_part");
			$num = $qty->num_rows();
		...
		$this->sisa = function($kode_part) {
			$stok = $this->db->query("SELECT kode_part, SUM(stock) as qty FROM cle_master_inventory WHERE kode_part='$kode_part' GROUP BY kode_part");
			$stok_keluar = $this->db->query("SELECT kode_part, SUM(quantity) as qty FROM cle_master_transaction_out WHERE kode_part='$kode_part' GROUP BY kode_part");
			$stok_masuk = $this->db->query("SELECT kode_part, SUM(quantity) as qty FROM cle_master_transaction_in WHERE kode_part='$kode_part' GROUP BY kode_part");
		...
		$this->kebutuhan = function($kode_part) {
			$qty = $this->db->query("SELECT kode_part, SUM(quantity) as qty FROM cle_master_require_part WHERE kode_part='$kode_part' GROUP BY kode_part");
			$num = $qty->num_rows();
	...
	public function get_preview($data) {
		...
		$atm = function($id, $field) {
			error_reporting(0);
			$atm = $this->db->query("SELECT $field FROM cle_master_atm WHERE idatm='$id'")->row();
			$atm = (array) $atm;
			return $atm[$field];
		};
		...
		$bank = function($id, $field) {
			error_reporting(0);
			$bank = $this->db->query("SELECT $field FROM cle_master_client WHERE id='$id'")->row();
			$bank = (array) $bank;
			return $bank[$field];
		};
		
		$pic = function($id) {
			$nama = $this->db->query("SELECT nama FROM cle_master_staff WHERE id='$id'")->row()->nama;
	...
	public function get_data_inv() {
		$result = $this->db->query("SELECT * FROM cle_master_ticket WHERE accept_time IS NOT NULL AND end_job IS NOT NULL");
	...
	public function generate($act) {
		...
		$result = $this->db->query("SELECT * FROM cle_master_ticket WHERE entry_date LIKE '".$_REQUEST['bulan']."%' AND accept_time IS NOT NULL AND end_job IS NOT NULL");
		...
		foreach($result->result() as $r) {
			...
			$without_breaks = str_replace(array('<li>'), '', $without_breaks);
			$without_breaks = str_replace(array('</li>'), ', ', $without_breaks);
			$without_breaks = str_replace(array('<div>', '</div>'), ', ', $without_breaks);
