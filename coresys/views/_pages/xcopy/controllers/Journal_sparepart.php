...
class Journal_sparepart extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		$this->data['rekap'] = function($merk) {
			return $this->data_summary($merk);
		};
		
	
		return view('pages/journal_sparepart/index', $this->data);
	}
	
	public function tes() {
		echo "<pre>";
		print_r($this->data_summary("NCR"));
	}
	
	public function data_summary($merk) {
		$q = "SELECT 
				b.nama_part, a.*, a.quantity as stock,
				...
				(SELECT sum(price) FROM master_inventory_part WHERE id_detail=a.id) as price,
				(SELECT COALESCE(SUM(quantity),0) FROM master_transaction_out WHERE kode_part=a.kode_part) AS quantity_out,
				((SELECT COUNT(*) FROM master_inventory_part WHERE id_detail=a.id) - (SELECT COALESCE(SUM(quantity),0) FROM master_transaction_out WHERE kode_part=a.kode_part)) AS quantity_in
			FROM master_transaction_in a LEFT JOIN master_inventory b ON(a.kode_part=b.kode_part) WHERE b.category!='' AND b.merk='$merk' GROUP BY a.kode_part";
			
		// $q = "SELECT b.nama_part, a.*, a.quantity as stock,
				// #(SELECT count(*) FROM master_inventory_part WHERE id_detail=a.id AND status='available') as available, 
				// #(SELECT count(*) FROM master_inventory_part WHERE id_detail=a.id AND status='used') as used,
				// (SELECT sum(price) FROM master_inventory_part WHERE id_detail=a.id) as price,
				// (SELECT COALESCE(SUM(quantity),0) FROM master_transaction_out WHERE kode_part=a.kode_part) AS quantity_out,
				// ((SELECT COUNT(*) FROM master_inventory_part WHERE id_detail=a.id) - (SELECT COALESCE(SUM(quantity),0) FROM master_transaction_out WHERE kode_part=a.kode_part)) AS quantity_in FROM master_inventory b LEFT JOIN master_transaction_in a ON(a.kode_part=b.kode_part)  WHERE b.category!='' AND b.merk='$merk' GROUP BY a.kode_part";
