...
class Dashboard_crmcr extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/dashboard_crmcr/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/dashboard_crmcr/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("dashboard_crmcr");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'dashboard_crmcr';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("dashboard_crmcr");
        $result = $query->row();
