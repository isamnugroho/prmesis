...
class Cro_master_news extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cro_master_news/index', $this->data);
	}
	...
	public function insert() {
		...
		if($id==null){
			...
			$result = $this->db->insert('cro_master_news', $data);
		} else {
			$data['title'] = $title;
			$data['information'] = $information;
			$this->db->where('id', $id);
			$result = $this->db->update('cro_master_news', $data);
		}
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
		$row = $this->db->get('cro_master_news')->row();
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('cro_master_news');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cro_master_news");
        $data = array();
		error_reporting(0);
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cro_master_news';
            $data[]= array(
                $no,
                $rows->title,
                $rows->information,
