...
class Cle_master_item extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cle_master_item/index', $this->data);
	}
	
	public function add() {
		return view('pages/cle_master_item/add');
	}
	
	public function edit() {
		return view('pages/cle_master_item/edit');
	}
	...
	public function insert() {
		...
		$res = $this->db->get('cle_master_item');
		$num = $res->num_rows();
		
		if($num>0) {
			$result['status'] = 'exist';
		} else {
			$res = $this->db->insert('cle_master_item', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_master_item');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cle_master_item");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cle_master_item';
			$url2 = 'cle_master_item_detail/index';
            $data[]= array(
                $no,
                $rows->name,
                // $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cle_master_item");
        $result = $query->row();
