...
class Cro_master_user extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cro_master_user/index', $this->data);
	}
	...
	public function add() {
		...
		return view('pages/cro_master_user/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("cro_master_user")->row();
	...
	public function insert() {
		...
		$data['level'] = $level;
		$result = $this->db->insert('cro_master_user', $data);
	...
	public function update() {
		...
		$data['level'] = $level;
		$this->db->where("id", $id);
		$result = $this->db->update('cro_master_user', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cro_master_user');
	...
	public function get_staff() {
		...
		$query = "SELECT * FROM master_staff WHERE (nik LIKE '$search' OR nama LIKE '$search') AND nik NOT IN (SELECT username FROM cro_master_user)";
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cro_master_user");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cro_master_user';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cro_master_user");
        $result = $query->row();
