...
class Cro_master_location extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cro_master_location/index', $this->data);
	}
	
	public function add() {
		return view('pages/cro_master_location/add');
	}
	
	public function edit() {
		return view('pages/cro_master_location/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('cro_master_location');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('cro_master_location', $data);
			...
			if($num>0) {
				...
				$this->db->where('id', $id);
				$res = $this->db->update('cro_master_location', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cro_master_location');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cro_master_location");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$count = function($id) {
				$res = $this->db->query("SELECT * FROM cro_master_location_detail WHERE id_lokasi='".$id."'");
				
				return $res->num_rows();
			};
			$cse = function($id) {
				$res = $this->db->query("SELECT * FROM trans_schedule 
											LEFT JOIN cro_master_location ON (trans_schedule.id_lokasi=cro_master_location.id) 
			...
			$no++;
			$url = base_url().'cro_master_location';
			$url2 = 'cro_master_location_detail/index';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cro_master_location");
        $result = $query->row();
