...
class Cle_master_item_detail extends MY_Controller {
	var $data = array();
	...
	public function index($id) {	
		$this->data['atm'] = $this->db->query("SELECT * FROM master_item WHERE id NOT IN (SELECT id_detail FROM cle_master_item_detail)")->result();
		$this->data['id'] = $id;
	
		return view('pages/cle_master_item_detail/index', $this->data);
	}
	
	public function add() {
		return view('pages/cle_master_item_detail/add');
	}
	
	public function edit() {
		return view('pages/cle_master_item_detail/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('cle_master_item_detail');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('cle_master_item_detail', $data);
			...
			if($num>0) {
				...
				$this->db->where('id', $id);
				$res = $this->db->update('cle_master_item_detail', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_master_item_detail');
	...
	function select_atm() {
		...
		// }
		$query = "SELECT * FROM master_atm WHERE id NOT IN (SELECT id_atm FROM cle_master_item_detail) AND (idatm LIKE '$search' OR cabang LIKE '$search' OR lokasi LIKE '$search')";
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cle_master_item_detail");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cle_master_item_detail';
			$url2 = base_url().'cle_master_item_detail';
            $data[]= array(
                $no,
                $rows->list,
                // $rows->username,
	...
	public function totalDatas($id) {
		$this->db->where('id_detail',$id);
		$query = $this->db->select("COUNT(*) as num")->get("cle_master_item_detail");
        $result = $query->row();
