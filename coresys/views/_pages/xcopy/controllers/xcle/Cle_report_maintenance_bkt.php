...
class Report_maintenance_bkt extends MY_Controller {
	var $data = array();
	...
	public function index() {
		return view('pages/report_maintenance_bkt/index', $this->data);
	}
	...
	function json() {
		...
		$query = "
			SELECT *,
			(SELECT nama FROM master_staff WHERE id=master_ticket.pic) as cse,
			(SELECT lokasi FROM master_atm WHERE idatm=master_ticket.atm_id) as lokasi,
			(SELECT idatm FROM master_atm WHERE idatm=master_ticket.atm_id) as idatm
			FROM master_ticket
		";
