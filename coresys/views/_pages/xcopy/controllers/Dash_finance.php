...
class Dash_finance extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/dash_finance/index', $this->data);
	}
	public function get_data_today() {
		$result = $this->db->query("SELECT * FROM finance_costing 
									LEFT JOIN master_ticket ON (finance_costing.atm_id=master_ticket.atm_id)
									WHERE master_ticket.entry_date LIKE '%".date("Y-m-d")."%'");
		...
		if($count>0) {
			foreach($result->result() as $r) {
				...
				$this->db->where('ticket_id', $r->ticket_id);
				$q = $this->db->get('finance_akomodation');
				if ( $q->num_rows() == 0 ) {
					$this->db->set('ticket_id', $r->ticket_id);
					$this->db->insert('finance_akomodation', $data);
				}
			}
		}
	}
	...
	public function get_data_dash() {
		$result = $this->db->query("SELECT *, UPPER(finance_akomodation.status) as status, master_staff.nama as nama 
									FROM finance_costing
									LEFT JOIN master_ticket ON (finance_costing.atm_id=master_ticket.atm_id)
									LEFT JOIN finance_akomodation ON (finance_akomodation.kode_costing=finance_costing.kode_costing)
									LEFT JOIN master_staff ON (master_staff.id=finance_costing.cse)
									WHERE master_ticket.entry_date LIKE '%".date("Y-m-d")."%'
									");
		$result = $this->db->query("SELECT * FROM finance_akomodation 
									...
									LEFT JOIN master_ticket ON (finance_akomodation.ticket_id=master_ticket.ticket_id)
									LEFT JOIN master_staff ON (master_staff.id=finance_costing.cse)
									WHERE master_ticket.entry_date LIKE '%".date("Y-m-d")."%'
		...
		foreach($result->result() as $r) {
			$data[] = array(
				...
				"ticket_id" => $r->ticket_id,
				"status" => $r->status,
				"action" => "<button onclick='window.location.href=\"".base_url()."costing_job\"' class='btn btn-xs'>Open Detail</button>"
			);
		}
	...
	public function index2() {	
		...
		return view('pages/dash_finance/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("dash_finance");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'dash_finance';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("dash_finance");
        $result = $query->row();
