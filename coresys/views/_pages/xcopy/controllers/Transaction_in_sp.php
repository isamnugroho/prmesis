...
class Transaction_in_sp extends MY_Controller {
	var $data = array();
	var $data_temp = array();
	...
	public function index() {	
		...
		return view('pages/master_transaction_in_sp/index', $this->data);
	}
	...
	public function add() {
		...
		return view('pages/master_transaction_in_sp/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("master_transaction_in_sp")->row();
	...
	public function insert() {
		...
		$data = array();
		if($id==null){
			$res = $this->db->query("SELECT * FROM master_transaction_in_sp WHERE tgl_terima='$tgl_terima' AND kode_part='$kode_part'");
			if($res->num_rows() > 0) {
				...
				$result = $this->db->update('master_transaction_in_sp', $data);
			} else {
				$data['tgl_terima'] = $tgl_terima;
				$data['kode_part'] = $kode_part;
				$data['quantity'] = $quantity;
				$result = $this->db->insert('master_transaction_in_sp', $data);
				if($result) {
	...
	public function update() {
		...
		redirect('master_transaction_in_sp');
	}
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('master_transaction_in_sp');
	...
	public function get_data_temp() {
		...
		$url = base_url().'transaction_in_sp';
		$data = array();
		$no = 0;
		foreach($data_temp as $k => $r) {
			$no++;
	...
	public function get_data($merk) {
		...
		if($level=="CSE") {
			$where = "WHERE a.id_location='$id_lokasi'";
		}
		...
		foreach($employees->result() as $rows) {
			...
			if($level!=="CSE") {
				$button_act = '<center><a onclick="updateModal(
					\''.$rows->ids.'\',
					\''.$rows->tgl_keluar.'\',
					\''.$rows->kode_part.'\',
