...
class New_ticket extends MY_Controller {
    ...
    public function ticket_new() {
        return view('pages/new_ticket/index_new', $this->data);
		// print_r($this->data['user_access']);
    ...
    public function close($id) {
		$this->db->where('id', $id);
		$this->db->update('master_ticket', array("status"=>"CLOSE"));
    ...
    public function close_manual() {
		...
		$data_n['status'] = "close";
		$this->db->where('ticket_id', $ticket_id);
		$result = $this->db->update('master_ticket', $data_n);
    ...
    public function get_data() {
		...
		$valid_columns = array(
            0=>'master_ticket.id',
            1=>'master_ticket.atm_id',
            2=>'master_atm.cabang',
            3=>'master_atm.lokasi',
            4=>'master_ticket.ticket_id',
            5=>'master_ticket.problem_type',
            6=>'master_ticket.entry_date',
        );
        ...
        $this->db->select('*,master_ticket.id as ids, master_ticket.status as status');
        $this->db->from('master_ticket');
		$this->db->join('master_atm', 'master_atm.idatm=master_ticket.atm_id');
	...
	public function json() {
		...
		// $query = "
			// SELECT *, master_ticket.id as ids, master_ticket.status as status, master_client.nama as nama_bank, master_ticket.pic as pic
			// FROM master_ticket 
		...
		$param['order'] 		= array('master_ticket.id' => 'desc');
		$param['where'] 		= array();
	...
	public function json2() {
		...
		$query = "
			SELECT *, master_ticket.id as ids, primary_table.status as status, master_client.nama as nama_bank, master_ticket.pic as pic FROM master_ticket_detail primary_table 
