...
class Notification extends MY_Controller {
	...
	public function get_notif_costing() {
		$res = $this->db->query("SELECT *, notify.id as id FROM notify LEFT JOIN master_ticket ON(master_ticket.id=notify.id_menu) LEFT JOIN finance_akomodation ON(finance_akomodation.ticket_id=master_ticket.ticket_id OR finance_akomodation.id=notify.id_menu) WHERE notify.status='unread' AND (notify.menu='costing_job' OR notify.menu='costing_rev') GROUP BY notify.menu, notify.id_menu ORDER BY notify.id DESC ")->result();
