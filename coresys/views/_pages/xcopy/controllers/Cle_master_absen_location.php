...
class Cle_master_absen_location extends MY_Controller {
	var $data = array();
	...
	public function index() {
		$this->data['absen_location'] = $this->db->get('cle_master_absen_location')->result();
		
		$location = $this->db->query("SELECT * FROM master_location WHERE id NOT IN (SELECT id_lokasi FROM cle_master_absen_location)")->result();
		$location_edit = $this->db->query("SELECT * FROM master_location")->result();
		$this->data['location'] = $location;
		$this->data['location_edit'] = $location_edit;
		
		return view('pages/cle_master_absen_location/index', $this->data);
	}
	...
	public function insert() {
		...
		if($id==null){
			...
			$result = $this->db->insert('cle_master_absen_location', $data);
		} else {
			$data['lokasi'] = $lokasi;
			...
			$data['keterangan'] = $keterangan;
			$this->db->where('id', $id);
			$result = $this->db->update('cle_master_absen_location', $data);
		}
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_master_absen_location');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cle_master_absen_location");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cle_master_absen_location';
            $data[]= array(
                $no,
                $rows->lokasi,
                $rows->latlng,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cle_master_absen_location");
        $result = $query->row();
