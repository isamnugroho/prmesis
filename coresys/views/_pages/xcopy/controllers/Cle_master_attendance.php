...
class Cle_master_attendance extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/cle_master_attendance/index', $this->data);
	}
	...
	public function add() {
		...
		return view('pages/cle_master_attendance/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("cle_master_attendance")->row();
	...
	public function attend_list() {
		...
		$first_in_check = function($employee_id, $attendance_date) {
			$sql = 'SELECT * FROM cle_master_attendance_time WHERE employee_id = ? and attendance_date = ? limit 1';
		...
		$first_out_check = function($employee_id, $attendance_date) {
			$sql = 'SELECT * FROM cle_master_attendance_time WHERE employee_id = ? and attendance_date = ? order by time_attendance_id desc limit 1';
			$binds = array($employee_id,$attendance_date);
			$query = $this->db->query($sql, $binds);
		...
		$first_in = function($employee_id, $attendance_date) {
			$sql = 'SELECT * FROM cle_master_attendance_time WHERE employee_id = ? and attendance_date = ?';
			$binds = array($employee_id, $attendance_date);
			$query = $this->db->query($sql, $binds);
		...
		$total_hours_worked_attendance = function($employee_id, $attendance_date) {
			$sql = 'SELECT * FROM cle_master_attendance_time WHERE employee_id = ? and attendance_date = ? and total_work != ""';
			$binds = array($employee_id, $attendance_date);
			$query = $this->db->query($sql, $binds);
		...
		// $data = function($param, $id, $date) {
			// if($param=="all"
				// $this->db->query("SELECT * FROM cle_master_attendance");
			// }
		// };
	...
	public function insert() {
		...
		$result = $this->db->insert('cle_master_attendance', $data);
		...
		redirect('cle_master_attendance');
	}
	...
	public function update() {
		...
		$data['password'] = $password;
		$this->db->where("id", $id);
		$result = $this->db->update('cle_master_attendance', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_master_attendance');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cle_master_attendance_time");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cle_master_attendance';
            $data[]= array(
                $no,
                $rows->employee_id,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cle_master_attendance_time");
        $result = $query->row();
