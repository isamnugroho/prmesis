...
class Cle_trans_staff extends MY_Controller {
	var $data = array();
	...
	{	
		return view('pages/cle_trans_staff/index', $this->data);
	}
	
	public function add() {
		return view('pages/cle_trans_staff/add');
	}
	
	public function edit() {
		return view('pages/cle_trans_staff/edit');
	}
	...
	public function insert() {
		...
		$res = $this->db->insert('cle_trans_staff', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('cle_trans_staff');
	...
	function select_petugas() {
		...
		// }
		$query = "SELECT * FROM master_petugas WHERE id NOT IN (SELECT id_petugas FROM cle_trans_staff) AND (nik LIKE '$search' OR nama LIKE '$search' OR username LIKE '$search')";
	...
	function select_lokasi() {
		...
		$search = "%".strtolower($search)."%";
		// }
		$query = "SELECT * FROM master_lokasi WHERE id NOT IN (SELECT id_lokasi FROM cle_trans_staff) AND id IN (SELECT id_lokasi FROM master_lokasi_detail) AND (name LIKE '$search')";
		$result = $this->db->query($query)->result();
	...
	public function get_data() {
        ...
        $employees = $this->db->get("cle_trans_staff");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'cle_trans_staff';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("cle_trans_staff");
        $result = $query->row();
