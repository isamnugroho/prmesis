...
class Daily_mon4 extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		$query = "SELECT * FROM view_historical_problem";
		$result = $this->db->query($query)->result_array();
		$this->data['historical_problem'] = $result;
	
		return view('pages/daily_mon4/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/daily_mon4/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("daily_mon4");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'daily_mon4';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("daily_mon4");
        $result = $query->row();
