...
class Dash_key extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/dash_key/index', $this->data);
	}
	
	public function data_sla() {
		$result = $this->db->query("SELECT * FROM master_request_key WHERE entry_date LIKE '2021-04%' AND accept_time IS NOT NULL AND end_job IS NOT NULL AND ticket_id='BST2104220032'");
		$result = $this->db->query("SELECT * FROM master_request_key WHERE bank_id='18' AND entry_date LIKE '".date("Y-m")."%' AND accept_time IS NOT NULL AND end_job IS NOT NULL");
...
// 			echo "<hr>";
	...
	public function interval_ticket() {
		...
		$query = "SELECT 
					*,
					...
					(SELECT count(*) FROM master_request_key WHERE entry_date=A.entry_date) as totticket,
					(SELECT count(*) FROM master_request_key WHERE entry_date=A.entry_date AND (status LIKE 'done' OR status LIKE 'close')) as tclose,
					(SELECT count(*) FROM master_request_key WHERE entry_date=A.entry_date AND status LIKE 'open') as topen
					FROM master_request_key as A 
	...
	public function dashboard_summary() {
		...
		// BANK MALUKU MALUT 19
		// BANK KALTENG 20
		$query_total_ticket = $this->db->query("SELECT * FROM master_request_key 
												LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
												LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)");
		$count_ticket = $query_total_ticket->num_rows();
		
		$query_total_ticket_open = $this->db->query("SELECT * FROM master_request_key 
													 LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
													 LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
													 WHERE master_request_key.status LIKE '%open%'");
		$count_ticket_open = $query_total_ticket_open->num_rows();
		
		$query_total_ticket_close = $this->db->query("SELECT * FROM master_request_key  
													 LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
													 LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
													 WHERE (master_request_key.status LIKE '%done%' OR master_request_key.status LIKE '%close%')");
		$count_ticket_close = $query_total_ticket_close->num_rows();
		
		
		
		$query_today_ticket = $this->db->query("SELECT * FROM master_request_key  
												LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
												LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
												WHERE master_request_key.entry_date LIKE '%".date("Y-m-d")."%'");
		$count_today_ticket = $query_today_ticket->num_rows();
		
		$query_today_ticket_open = $this->db->query("SELECT * FROM master_request_key  
													 LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
													 LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
													 WHERE master_request_key.status LIKE '%open%' AND master_request_key.entry_date LIKE '%".date("Y-m-d")."%'");
		$count_today_ticket_open = $query_today_ticket_open->num_rows();
		
		$query_today_ticket_close = $this->db->query("SELECT * FROM master_request_key  
													 LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
													 LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
													 WHERE (master_request_key.status LIKE '%done%' OR master_request_key.status LIKE '%close%') AND master_request_key.entry_date LIKE '%".date("Y-m-d")."%'");
		$count_today_ticket_close = $query_today_ticket_close->num_rows();
														
		$result = array(
			'total_request' => $count_ticket,
			'total_open_request' => $count_ticket_open,
			'total_close_request' => $count_ticket_close,
			'total_today_request' => $count_today_ticket,
			'total_today_open_request' => $count_today_ticket_open,
			'total_today_close_request' => $count_today_ticket_close,
		);
	...
	public function summary_ticket() {
		...
		$res = $this->db->query("SELECT *, master_request_key.status as status, master_request_key.pic as pic FROM master_request_key 
									LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
									LEFT JOIN master_client ON (master_client.id=master_request_key.bank_id) ORDER BY master_request_key.entry_date DESC");
		...
		foreach($res->result() as $rows) {
			...
			$arry[] = array(
				...
				'key_open' => $rows->key_open,
				'key_close' => $rows->key_close,
	...
	public function get_week_data() {
		...
		foreach($daterange as $date) {
			...
			$ticket = $this->db->query("SELECT count(*) as cnt FROM master_request_key 
										LEFT JOIN master_atm ON (master_atm.id=master_request_key.atm_id)
										LEFT JOIN master_client ON (master_client.id=master_atm.id_bank)
										WHERE master_client.nama='$bank' AND master_request_key.entry_date LIKE '$date%'");
