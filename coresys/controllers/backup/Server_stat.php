...
class Server_stat extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/server_stat/index', $this->data);
	}
	
	public function add() {
		return view('pages/server_stat/add');
	}
	
	public function edit() {
		return view('pages/server_stat/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('server_stat');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('server_stat', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('server_stat', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('server_stat');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("server_stat");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'server_stat';
			$url2 = base_url().'server_stat';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("server_stat");
        $result = $query->row();
