...
class New_ticketflm extends MY_Controller {
    var $data = array();
    ...
    public function index() {
        return view('pages/new_ticketflm/index', $this->data);
    ...
    public function ticket_new() {
        return view('pages/new_ticketflm/index_new', $this->data);
		// print_r($this->data['user_access']);
			...
			if($key==0) {
				...
				if($res->num_rows()==0) {
					...
					if($code==0) {
						...
						echo "KODE : ".$kode;
						echo "<br>QUERY : "."SELECT ticket_id FROM flm_master_ticket WHERE ticket_id LIKE '%$check%' ORDER BY ticket_id DESC<br>";
				...
				if ( $q->num_rows() > 0 ) {
					...
					$this->db->insert('flm_master_ticket', $data);
				}
			}
		}
    ...
    public function insert() {
        ...
        $res = $this->db->insert('flm_master_ticket', $data);
    ...
    public function close($id) {
		$this->db->where('id', $id);
		$this->db->update('flm_master_ticket', array("status"=>"CLOSE"));
    ...
    public function close_manual() {
		...
		$data_n['status'] = "close";
		$this->db->where('ticket_id', $ticket_id);
		$result = $this->db->update('flm_master_ticket', $data_n);
    ...
    public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('flm_master_ticket');
    ...
    function get_ticket2() {
        $leading = "T".date('Ym');
        $check = date('Ym');
        $res = $this->db->query("SELECT ticket_id FROM flm_master_ticket WHERE ticket_id LIKE '%$check%' ORDER BY ticket_id DESC");
        if($res->num_rows()==0) {
            $kode = 1;
        } else {
            $kode = $res->row()->ticket_id;
    ...
    function select_cse() {
		...
		$query = "SELECT *, master_staff.id as ids FROM master_staff 
					...
					WHERE master_user.level='flm' AND master_location.id_bank='$id_bank' AND (master_staff.nama LIKE '$search') GROUP BY master_staff.id";
		$result = $this->db->query($query)->result();
    ...
    public function get_data() {
		...
		$valid_columns = array(
            0=>'flm_master_ticket.id',
            1=>'flm_master_ticket.atm_id',
            2=>'master_atm.cabang',
            3=>'master_atm.lokasi',
            4=>'flm_master_ticket.ticket_id',
            5=>'flm_master_ticket.problem_type',
            6=>'flm_master_ticket.entry_date',
        );
        ...
        $this->db->select('*,flm_master_ticket.id as ids, flm_master_ticket.status as status');
        $this->db->from('flm_master_ticket');
		$this->db->join('master_atm', 'master_atm.idatm=flm_master_ticket.atm_id');
    ...
    public function totalDatas() {
        $query = $this->db->select("COUNT(*) as num")->get("flm_master_ticket");
        $result = $query->row();
	...
	public function json() {
		...
		// $query = "
			// SELECT *, flm_master_ticket.id as ids, flm_master_ticket.status as status, master_client.nama as nama_bank, flm_master_ticket.pic as pic
			// FROM flm_master_ticket 
			// LEFT JOIN master_atm ON (master_atm.idatm=flm_master_ticket.atm_id)
			// LEFT JOIN master_client ON (master_client.id=flm_master_ticket.bank_id)
		// ";
		...
		$param['order'] 		= array('flm_master_ticket.id' => 'desc');
		$param['where'] 		= array();
	...
	public function json2() {
		...
		$query = "
			SELECT *, flm_master_ticket.id as ids, primary_table.status as status, master_client.nama as nama_bank, flm_master_ticket.pic as pic FROM flm_master_ticket_detail primary_table 
