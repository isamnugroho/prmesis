...
class Docmain extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/docmain/index', $this->data);
	}
	
	public function add() {
		return view('pages/docmain/add');
	}
	
	public function edit() {
		return view('pages/docmain/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('docmain');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('docmain', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('docmain', $data);
	...
	public function get_data() {
        ...
        $employees = $this->db->get("docmain");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'docmain';
			$url2 = base_url().'docmain';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("docmain");
        $result = $query->row();
