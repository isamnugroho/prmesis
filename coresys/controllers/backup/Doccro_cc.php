...
class Doccro_cc extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/doccro_cc/index', $this->data);
	}
	
	public function add() {
		return view('pages/doccro_cc/add');
	}
	
	public function edit() {
		return view('pages/doccro_cc/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('doccro_cc');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('doccro_cc', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('doccro_cc', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('doccro_cc');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("doccro_cc");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'doccro_cc';
			$url2 = base_url().'doccro_cc';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("doccro_cc");
        $result = $query->row();
