...
class Transaction_in extends MY_Controller {
	var $data = array();
	var $data_temp = array();
	...
	public function index() {	
		...
		return view('pages/master_transaction_in/index', $this->data);
	}

	public function get_category() {
		$array = $this->db->query('SELECT * FROM master_inventory WHERE kode_part="'.$_REQUEST['id'].'"')->row();
	...
	public function add() {
		...
		return view('pages/master_transaction_in/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("master_transaction_in")->row();
	...
	public function insert() {
		...
		$data = array();
		if($id==null){
			$res = $this->db->query("SELECT * FROM master_transaction_in WHERE tgl_terima='$tgl_terima' AND kode_part='$kode_part'");
			if($res->num_rows() > 0) {
				...
				$result = $this->db->insert('master_transaction_in', $data);
				if($result) {
	...
	public function update() {
		...
		redirect('master_transaction_in');
	}
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('master_transaction_in');
	...
	public function get_data($merk) {
		...
		// $query = "SELECT *, a.id as ids FROM master_transaction_in a LEFT JOIN master_inventory b ON(a.kode_part=b.id) $where";
		$query = "SELECT *, a.id as ids, a.quantity, count(a.id) as cnt,
				  b.merk as merk FROM master_transaction_in a
