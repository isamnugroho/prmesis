...
class Transaction_out extends MY_Controller {
	...
	public function get_part_detail() {
		...
		$query = "
			...
			LEFT JOIN master_inventory c ON (b.kode_part=c.kode_part) 
			WHERE c.merk='".$merk."' AND c.kode_part='".$part."' AND a.status='available' AND (a.sn_part LIKE '$search')
		";
		$result = $this->db->query($query)->result();
	...
	public function insert_temp($id_detail) {
		if(count($_SESSION['temp'])!=0) {
			...
			$query = "SELECT * FROM master_transaction_out WHERE id='$id_detail'";
			// echo $query."<br>";
