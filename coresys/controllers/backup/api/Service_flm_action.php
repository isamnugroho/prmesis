...
class Service_flm_action extends REST_Controller {
	...
	function new_jobX_get() {
		...
		$query = "
			SELECT *, flm_master_ticket.id as ids, primary_table.status as status, master_client.nama as nama_bank FROM flm_master_ticket_detail primary_table 
			LEFT JOIN flm_master_ticket ON(flm_master_ticket.ticket_id=primary_table.ticket_id)
			LEFT JOIN master_atm ON(master_atm.idatm=flm_master_ticket.atm_id)
			LEFT JOIN master_client ON(master_client.id=master_atm.id_bank), 
			(SELECT MAX(id) as pId FROM flm_master_ticket_detail GROUP BY ticket_id) second_table
	...
	function job_list_get() {
		...
		// $query = "
			// SELECT master_atm.*, flm_master_ticket.*, flm_master_ticket.id as ids, master_client.nama as nama_bank FROM flm_master_ticket
			// LEFT JOIN flm_master_ticket_detail ON(flm_master_ticket_detail.ticket_id=flm_master_ticket.ticket_id)
			// LEFT JOIN master_atm ON(master_atm.idatm=flm_master_ticket.atm_id)
			// LEFT JOIN master_client ON(master_client.id=master_atm.id_bank)
			// WHERE flm_master_ticket.pic='".$id_user."' AND flm_master_ticket.accept_time IS NOT NULL AND flm_master_ticket.end_job IS NULL AND flm_master_ticket.status!='pending-pekerjaan'
		// ";
	...
	function new_job_detail_get() {
		...
		$query = "
			SELECT *, primary_table.id as ids, primary_table.status as status, master_client.nama as nama_bank FROM flm_master_ticket_detail primary_table 
			LEFT JOIN flm_master_ticket ON(flm_master_ticket.ticket_id=primary_table.ticket_id)
			LEFT JOIN master_atm ON(master_atm.idatm=flm_master_ticket.atm_id)
			LEFT JOIN master_client ON(master_client.id=master_atm.id_bank), 
			(SELECT MAX(id) as pId FROM flm_master_ticket_detail GROUP BY ticket_id) second_table
			WHERE primary_table.id = second_table.pId 
			AND primary_table.pic='$id_user' AND flm_master_ticket.id='$id'
		";
	...
	function job_pending_get() {
		...
		$query = "
			SELECT *, flm_master_ticket.id as id, primary_table.status as status FROM flm_master_ticket_detail primary_table 
			LEFT JOIN flm_master_ticket ON(flm_master_ticket.ticket_id=primary_table.ticket_id)
			LEFT JOIN master_atm ON(master_atm.idatm=flm_master_ticket.atm_id), 
			(SELECT MAX(id) as pId FROM flm_master_ticket_detail GROUP BY ticket_id) second_table
			WHERE primary_table.id = second_table.pId 
	...
	function job_done_get() {
		...
		$query = "
			...
			AND flm_master_ticket.entry_date > '2021-04-30 23:59:59'
		";
	...
	function crud_action($act) {
		...
		$detail = $this->db->query("SELECT *, flm_master_ticket_detail.id as id_detail FROM flm_master_ticket LEFT JOIN flm_master_ticket_detail ON (flm_master_ticket_detail.ticket_id=flm_master_ticket.ticket_id) WHERE flm_master_ticket.id='$id' ORDER BY flm_master_ticket_detail.id DESC")->row();
		
		$num1 = $this->db->query("SELECT * FROM flm_master_ticket_detail WHERE ticket_id='$detail->ticket_id' AND status='OPEN'");
		
		if($num1->num_rows()==0) {
			$data2['ticket_id'] = $detail->ticket_id;
			$data2['pic'] = $detail->pic;
			$this->db->insert('flm_master_ticket_detail', $data2);
		}
		
		$num = $this->db->query("SELECT * FROM flm_master_ticket_detail WHERE ticket_id='$detail->ticket_id' AND status='OPEN'");
		...
		if($act=="acc") {
			...
			$this->db->update('flm_master_ticket', $data);
			
			$this->db->set('accept_time', $time);
			$this->db->set('updated', 'true');
			$this->db->where('id', $num->row()->id);
			$res = $this->db->update('flm_master_ticket_detail');
		} else if($act=="arrive") {
	...
	function save_akomodation_get() {
		...
		$detail = $this->db->query("SELECT * FROM flm_master_ticket_detail WHERE ticket_id='$ticket_id' ORDER BY id DESC")->row();
	...
	public function sync_data_ticket_get() {
		...
		$res = $this->db->update_batch('flm_master_ticket', $updateArray, 'id'); 
	...
	public function sync_data_ticket_detail_get() {
		...
		$res = $this->db->update_batch('flm_master_ticket_detail', $updateArray, 'id'); 
	...
	public function get_history_post() {
		...
		$query = "
			SELECT * FROM flm_master_ticket LEFT JOIN master_staff ON (master_staff.id=flm_master_ticket.pic) WHERE atm_id='$idatm' AND flm_master_ticket.end_job IS NOT NULL ORDER BY flm_master_ticket.id DESC
		";
