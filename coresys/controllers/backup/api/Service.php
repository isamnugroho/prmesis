...
class Service extends REST_Controller {
	...
	function pending_jobcard_get() {
		...
		$query = "
			SELECT * FROM master_ticket WHERE 
			ticket_id NOT IN (SELECT ticket_id FROM trans_jobcard) AND
			master_ticket.pic='".$id_user."' AND master_ticket.entry_date > '2021-04-30 23:59:59'
			ORDER BY master_ticket.id DESC
		";
		
		$query = "
			SELECT * FROM master_ticket WHERE 
			ticket_id NOT IN (SELECT ticket_id FROM trans_jobcard) AND master_ticket.entry_date > '2021-04-30 23:59:59'
			ORDER BY master_ticket.id DESC
		";
	...
	function get_list_service3_get() {
		$this->db->select('*');
		$this->db->from('master_item');
		$query1 = $this->db->get();
		...
		$data = array(
			"master_item" => $query1->result(),
			"master_item_detail" => $query2->result()
		);
		
		echo json_encode($data);
	}

	function get_list_service2_get() {
		ini_set('display_errors', 1);
		ini_set('display_startup_errors', 1);
		error_reporting(E_ALL);
		$result = array();
		...
		foreach($res as $row) {
			...
			foreach($res as $r) {
				// if($row->name=="CHECK LIST ACTION TAKEN") {
					$data[$row->id_key][$i]['id'] = $r->id;
					$data[$row->id_key][$i]['id_detail'] = $r->id_detail;
					$data[$row->id_key][$i]['list'] = $r->list;
				// } else {
	...
	function get_data_ticket_get() {
		...
		$this->db->select('*');
		$this->db->from('master_ticket_detail');
