...
class Cle_action extends REST_Controller {
    ...
    function dashboard2_get() {
		...
		$query = $this->db->query("
			SELECT * FROM cle_master_atm
				LEFT JOIN cle_master_location_detail ON(cle_master_location_detail.id_atm=cle_master_atm.id)
				LEFT JOIN cle_trans_clean ON(cle_trans_clean.id_detail=cle_master_location_detail.id AND date='".date("Y-m-d")."')
				LEFT JOIN cle_trans_schedule ON(cle_trans_schedule.id_lokasi=cle_master_location_detail.id_lokasi)
				WHERE cle_trans_schedule.id_petugas='".$id_user."'
		");
		$num1 = $query->num_rows();
		
		// echo $num1."<br>";
		
		$query = $this->db->query("SELECT * FROM cle_trans_clean WHERE date='".date("Y-m-d")."' AND id_petugas='".$id_user."'");
		$num2 = $query->num_rows();
	...
	function dashboard_get() {
		...
		$query = $this->db->query("
			SELECT * FROM cle_master_atm
				LEFT JOIN cle_master_location_detail ON(cle_master_location_detail.id_atm=cle_master_atm.id)
				LEFT JOIN cle_trans_clean ON(cle_trans_clean.id_detail=cle_master_location_detail.id AND date='".date("Y-m-d")."' AND jenis_pekerjaan='PAGI')
				LEFT JOIN cle_trans_schedule ON(cle_trans_schedule.id_lokasi=cle_master_location_detail.id_lokasi)
				WHERE cle_trans_schedule.id_petugas='".$id_user."'
		");
		...
		$query2 = $this->db->query("
			SELECT * FROM cle_master_atm
				LEFT JOIN cle_master_location_detail ON(cle_master_location_detail.id_atm=cle_master_atm.id)
				LEFT JOIN cle_trans_schedule ON(cle_trans_schedule.id_lokasi=cle_master_location_detail.id_lokasi)
				WHERE cle_master_location_detail.id IN (SELECT id_detail FROM cle_trans_clean WHERE date='".date("Y-m-d")."' AND jenis_pekerjaan='PAGI') AND cle_trans_schedule.id_petugas='".$id_user."'
		");
		...
		$query = $this->db->query("
			SELECT * FROM cle_master_atm
				LEFT JOIN cle_master_location_detail ON(cle_master_location_detail.id_atm=cle_master_atm.id)
				LEFT JOIN cle_trans_clean ON(cle_trans_clean.id_detail=cle_master_location_detail.id AND date='".date("Y-m-d")."' AND jenis_pekerjaan='SORE')
				LEFT JOIN cle_trans_schedule ON(cle_trans_schedule.id_lokasi=cle_master_location_detail.id_lokasi)
				WHERE cle_trans_schedule.id_petugas='".$id_user."'
		");
		...
		$query2 = $this->db->query("
			SELECT * FROM cle_master_atm
				LEFT JOIN cle_master_location_detail ON(cle_master_location_detail.id_atm=cle_master_atm.id)
				LEFT JOIN cle_trans_schedule ON(cle_trans_schedule.id_lokasi=cle_master_location_detail.id_lokasi)
				WHERE cle_master_location_detail.id IN (SELECT id_detail FROM cle_trans_clean WHERE date='".date("Y-m-d")."' AND jenis_pekerjaan='SORE') AND cle_trans_schedule.id_petugas='".$id_user."'
		");
	...
	function new_job_get() {
		...
		$this->db->select('*,cle_master_location_detail.id as id_detail');
		$this->db->from('cle_master_location_detail');
		$this->db->where('id_lokasi', $id_lokasi);
		$this->db->join('cle_master_atm', 'cle_master_location_detail.id_atm = cle_master_atm.id');
		...
		if($status_jam_kerja=='true') {
			$query = "
				SELECT 
					*,  
					cle_master_location_detail.id as id_detail
					FROM 
						cle_master_location_detail
							LEFT JOIN cle_master_atm ON(cle_master_location_detail.id_atm = cle_master_atm.id)
							WHERE 
								id_lokasi='$id_lokasi' AND
								cle_master_location_detail.id NOT IN (SELECT id_detail FROM cle_trans_clean WHERE jenis_pekerjaan='$status_pekerjaan' AND date='$date')
			";
				...
				*,  
				...
				FROM 
					cle_master_location_detail
						...
						WHERE 
							id_lokasi='$id_lokasi' AND
							cle_master_location_detail.id = '$id'
		";
					...
					*,  
					...
					FROM 
						cle_master_location_detail
							...
							WHERE 
								id_lokasi='$id_lokasi' AND
								cle_master_atm.idatm = '$id_atm' AND
								cle_master_location_detail.id NOT IN (SELECT id_detail FROM cle_trans_clean WHERE id_petugas='$id_user' AND date='".date('Y-m-d')."' AND jenis_pekerjaan='$status_pekerjaan')
	...
	function save_job_get() {
		...
		if ($num_rows == 0) {
			$this->db->insert('cle_trans_clean', $data_simpan);
