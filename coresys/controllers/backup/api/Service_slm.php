...
class Service_slm extends REST_Controller {
	...
	function dashboard_get() {
		...
		$query1 = $this->db->query("SELECT * FROM slm_master_ticket WHERE entry_date LIKE '%".date("Y-m-d")."%' AND pic='".$id_user."'");
		$num1 = $query1->num_rows();
		
		$query2 = $this->db->query("SELECT * FROM slm_master_ticket WHERE entry_date LIKE '%".date("Y-m-d")."%' AND pic='".$id_user."'");
	...
	function new_job_get() {
		...
		$query = "
			...
			LEFT JOIN master_atm ON(master_atm.idatm=slm_master_ticket.atm_id)
			LEFT JOIN master_client ON(master_client.id=master_atm.id_bank)
			WHERE slm_master_ticket.pic='".$id_user."' AND accept_time IS NOT NULL AND end_job IS NULL
		";
	...
	function pending_jobcard_get() {
		...
		$query = "
			SELECT * FROM slm_master_ticket WHERE 
			ticket_id NOT IN (SELECT ticket_id FROM trans_jobcard) AND
			slm_master_ticket.pic='".$id_user."' AND slm_master_ticket.entry_date > '2021-04-30 23:59:59'
			ORDER BY slm_master_ticket.id DESC
		";
		
		$query = "
			SELECT * FROM slm_master_ticket WHERE 
			ticket_id NOT IN (SELECT ticket_id FROM trans_jobcard) AND slm_master_ticket.entry_date > '2021-04-30 23:59:59'
			ORDER BY slm_master_ticket.id DESC
		";
	...
	function acc_newjob_get() {
		...
		if($tz=="WIB" || $tz=="WITA" || $tz=="WIT") {
			...
			$this->db->where('id', $id);
			$res = $this->db->update('slm_master_ticket');
	...
	function job_detail_get() {
		...
		$query = "
			SELECT master_atm.*, slm_master_ticket.*, slm_master_ticket.id as ids, master_client.nama as nama_bank FROM slm_master_ticket
			LEFT JOIN master_atm ON(master_atm.id=slm_master_ticket.atm_id)
			LEFT JOIN master_client ON(master_client.id=master_atm.id_bank)
	...
	function save_job_post() {
		...
		if($saved['status']=='pending-pekerjaan') {
			$r = $this->db->query("SELECT * FROM slm_master_ticket WHERE id='$id_detail'")->row();
			...
			$this->db->insert('slm_master_ticket_extends', $extends_data);
		}
	...
	function get_data_ticket_get() {
		...
		$this->db->select('*');
		$this->db->from('slm_master_ticket_detail');
	...
	function check_sparepart_post() {
		...
		if($query->num_rows()==0) {
			...
			if($check->num_rows()) {
				...
				$ticket = $this->db->query("SELECT * FROM slm_master_ticket WHERE id='$id_detail'")->row();
