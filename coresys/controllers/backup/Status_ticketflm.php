...
class Status_ticketflm extends MY_Controller {
    var $data = array();
    ...
    public function index() {
		...
		$this->data['date'] = (isset($_REQUEST['id']) ? date("Y-m-d", strtotime($this->db->query("SELECT entry_date FROM flm_master_ticket WHERE id='".$_REQUEST['id']."'")->row()->entry_date)) : date('Y-m-d'));
        return view('pages/status_ticketflm/index', $this->data);
	}
    ...
    function update_status() {
		$sql = "UPDATE `flm_master_ticket` SET `updated`='false' WHERE 1";
    ...
    public function show_table() {
        $where_id = ($_GET['id']!=="" ? "AND flm_master_ticket.id='".$_GET['id']."'" : "");
        $date = (isset($_GET['date']) ? $_GET['date'] : date("Y-m-d"));
        // $query = "SELECT * FROM flm_master_ticket WHERE entry_date LIKE '%".$date."%' $where_id";
        $query = "SELECT 
					*, 
					flm_master_ticket.id as ids, 
					primary_table.status as status, 
					master_client.nama as nama_bank, 
					flm_master_ticket.pic as pic 
				FROM flm_master_ticket_detail primary_table 
				LEFT JOIN (SELECT id, atm_id, ticket_id, problem_type, entry_date, pic FROM flm_master_ticket) as flm_master_ticket ON(flm_master_ticket.ticket_id=primary_table.ticket_id)
				...
				WHERE primary_table.id = second_table.pId AND flm_master_ticket.entry_date LIKE '%".$date."%' $where_id";
