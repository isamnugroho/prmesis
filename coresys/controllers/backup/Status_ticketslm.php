...
class Status_ticketslm extends MY_Controller {
    var $data = array();
    ...
    public function index() {
		...
		$this->data['date'] = (isset($_REQUEST['id']) ? date("Y-m-d", strtotime($this->db->query("SELECT entry_date FROM master_ticket WHERE id='".$_REQUEST['id']."'")->row()->entry_date)) : date('Y-m-d'));
        return view('pages/status_ticketslm/index', $this->data);
	}
    ...
    public function show_table() {
        $where_id = ($_GET['id']!=="" ? "AND master_ticket.id='".$_GET['id']."'" : "");
        $date = (isset($_GET['date']) ? $_GET['date'] : date("Y-m-d"));
        // $query = "SELECT * FROM master_ticket WHERE entry_date LIKE '%".$date."%' $where_id";
        $query = "SELECT 
					*, 
				...
				LEFT JOIN (SELECT id, atm_id, ticket_id, problem_type, entry_date, pic FROM master_ticket) as master_ticket ON(master_ticket.ticket_id=primary_table.ticket_id)
				...
				WHERE primary_table.id = second_table.pId AND master_ticket.entry_date LIKE '%".$date."%' $where_id";
