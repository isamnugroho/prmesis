...
class Trans_schedule extends MY_Controller {
	...
	function select_petugas() {
		...
		// }
		$query = "SELECT * FROM master_staff LEFT JOIN master_user ON(master_staff.nik=master_user.username) WHERE (master_user.level='CSE' OR master_user.level='FLM' OR master_user.level='SLM') AND master_staff.id NOT IN (SELECT id_petugas FROM trans_schedule) AND (master_staff.nik LIKE '$search' OR master_staff.nama LIKE '$search' OR master_user.username LIKE '$search')";
