...
class Flm_doc extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/flm_doc/index', $this->data);
	}
	
	public function add() {
		return view('pages/flm_doc/add');
	}
	
	public function edit() {
		return view('pages/flm_doc/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('flm_doc');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('flm_doc', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('flm_doc', $data);
	...
	public function get_data() {
        ...
        $employees = $this->db->get("flm_doc");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'flm_doc';
			$url2 = base_url().'flm_doc';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("flm_doc");
        $result = $query->row();
