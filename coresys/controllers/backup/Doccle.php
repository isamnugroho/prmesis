...
class Doccle extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/doccle/index', $this->data);
	}
	
	public function add() {
		return view('pages/doccle/add');
	}
	
	public function edit() {
		return view('pages/doccle/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('doccle');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('doccle', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('doccle', $data);
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("doccle");
        $result = $query->row();
