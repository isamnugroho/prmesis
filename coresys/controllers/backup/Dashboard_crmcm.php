...
class Dashboard_crmcm extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/dashboard_crmcm/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/dashboard_crmcm/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("dashboard_crmcm");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'dashboard_crmcm';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("dashboard_crmcm");
        $result = $query->row();
