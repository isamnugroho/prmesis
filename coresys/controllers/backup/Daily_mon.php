...
class Daily_mon extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/daily_mon/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/daily_mon/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("daily_mon");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'daily_mon';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("daily_mon");
        $result = $query->row();
