...
class Master_location_detail extends MY_Controller {
	...
	function select_atm() {
		...
		// }
		$query = "SELECT * FROM master_atm WHERE id NOT IN (SELECT id_atm FROM master_location_detail) AND (idatm LIKE '$search' OR sn_mesin LIKE '$search' OR cabang LIKE '$search' OR lokasi LIKE '$search')";
