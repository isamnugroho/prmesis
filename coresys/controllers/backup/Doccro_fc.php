...
class doccro_fc extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/doccro_fc/index', $this->data);
	}
	
	public function add() {
		return view('pages/doccro_fc/add');
	}
	
	public function edit() {
		return view('pages/doccro_fc/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('doccro_fc');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('doccro_fc', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('doccro_fc', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('doccro_fc');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("doccro_fc");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'doccro_fc';
			$url2 = base_url().'doccro_fc';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("doccro_fc");
        $result = $query->row();
