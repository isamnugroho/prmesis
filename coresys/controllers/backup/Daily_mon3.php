...
class Daily_mon3 extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		$query = "SELECT * FROM view_position_cash";
		$result = $this->db->query($query)->result_array();
		$this->data['position_cash'] = $result;
	
		return view('pages/daily_mon3/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/daily_mon3/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("daily_mon3");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'daily_mon3';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("daily_mon3");
        $result = $query->row();
