...
class Flm_checklist extends MY_Controller {
    var $data = array();
    ...
    public function index() {
        return view('pages/flm_checklist/index', $this->data);
    }
    ...
    public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('flm_checklist');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('flm_checklist', $data);
			...
			if($num>0) {
				...
				$this->db->where('id', $id);
				$res = $this->db->update('flm_checklist', $data);
    ...
    public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('flm_checklist');
    ...
    public function totalDatas() {
        $query = $this->db->select("COUNT(*) as num")->get("flm_checklist");
        $result = $query->row();
