...
class Master_client extends MY_Controller {
	...
	public function insert() {
		...
		if($id==null) {
			...
			// if($num>0) {
				// $result['status'] = 'exist';
			// } else {
				$data_old = $query = $this->db->get_where('master_client', array('id' => $id), 1, 0)->row();
				$this->db->where('id', $id);
	...
	public function delete($id) {
		$data_old = $this->db->get_where('master_client', array('id' => $id), 1, 0)->row();
		$this->db->where('id', $id);
	...
	public function get_data() {
		...
		foreach($employees->result() as $rows) {
            ...
            $data[]= array(
                ...
                $rows->pic,
                '<center>
				<a onclick="deleteAction2(\''.$url.'/delete/'.$rows->id.'\')" class="btn btn-danger btn-sm zoomsmall" style="background: linear-gradient(to bottom, #00c6ff, #0072ff);border-radius: 4px;font-weight:bold;"><img style="float: left; margin: 1px 5px 0px 0px; height:16px; width:16px; " src="'.base_url().'assets/img/search.png"/>Detail</a>
				<a onclick="updateModal(
					...
					\''.$rows->kode_ticket.'\',
					\''.$rows->telp.'\'
				)" class="btn btn-warning btn-sm zoomsmall" style="background: linear-gradient(to bottom, #fe8c00, #f83600);border-radius: 4px;font-weight:bold;"><img style="float: left; margin: 1px 5px 0px 0px; height:16px; width:16px; " src="'.base_url().'seipkon/assets/img/edit.png"/>Edit</a>
                 <a onclick="deleteAction2(\''.$url.'/delete/'.$rows->id.'\')" class="btn btn-danger btn-sm zoomsmall" style="background: linear-gradient(to top, #ed213a, #93291e);border-radius: 4px;font-weight:bold;"><img style="float: left; margin: 1px 5px 0px 0px; height:16px; width:16px; " src="'.base_url().'seipkon/assets/img/del.png"/>Delete</a></center>'
            );     
        }
