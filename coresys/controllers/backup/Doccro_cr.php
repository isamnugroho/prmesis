...
class Doccro_cr extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		return view('pages/doccro_cr/index', $this->data);
	}
	
	public function add() {
		return view('pages/doccro_cr/add');
	}
	
	public function edit() {
		return view('pages/doccro_cr/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('doccro_cr');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('doccro_cr', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('doccro_cr', $data);
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('doccro_cr');
	...
	public function get_data() {
        ...
        $employees = $this->db->get("doccro_cr");
        $data = array();
		
		$no = $start;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'doccro_cr';
			$url2 = base_url().'doccro_cr';
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("doccro_cr");
        $result = $query->row();
