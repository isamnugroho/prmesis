...
class Slm_checklist extends MY_Controller {
    var $data = array();
    ...
    public function index() {
        return view('pages/slm_checklist/index', $this->data);
    }
    ...
    public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('slm_checklist');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('slm_checklist', $data);
			...
			if($num>0) {
				...
				$this->db->where('id', $id);
				$res = $this->db->update('slm_checklist', $data);
    ...
    public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('slm_checklist');
    ...
    public function totalDatas() {
        $query = $this->db->select("COUNT(*) as num")->get("slm_checklist");
        $result = $query->row();
