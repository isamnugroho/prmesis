	...
	public function sync_complaint_post() {
		...
		if(isset($_REQUEST['data']['sync'])) {
			...
			foreach($output as $k => $r) {
				foreach($r as $s) {
					$sql = "
						SELECT *, trans_complaint.date as date, trans_complaint_detail.id as id_sync FROM trans_complaint_detail 
						...
						WHERE master_kelolaan_detail.tid='".$s['tid']."' AND trans_complaint_detail.timestamp LIKE '%$k%'
					";
	...
	public function get_scheduled_post() {
		...
		if($authorized) {
			...
			if($result_schedule->num_rows()) {
				...
				if($is_koord=="true") {
					$sql = "
						SELECT *, 'rutin' as jenis_job, trans_clean.date as date, trans_clean_detail.id as id_report, master_staff.nama as nama_petugas 
					...
					$sql = "
						SELECT *, 'rutin' as jenis_job, trans_clean.date as date, trans_clean_detail.id as id_report, master_staff_petugas.nama as nama_petugas 
						FROM trans_schedule 
					...
					if($check_switch>0) {
						$sql2 = "
							SELECT *, 'switch' as jenis_job, trans_clean.date as date, trans_clean_detail.id as id_report, master_staff_petugas.nama as nama_petugas
							 FROM trans_schedule
							 LEFT JOIN trans_schedule_team ON(trans_schedule_team.id_detail=trans_schedule.id) 
							 LEFT JOIN master_kelolaan_detail ON(master_kelolaan_detail.tid=trans_schedule_team.tid) 
							 LEFT JOIN master_atm ON(master_atm.tid=trans_schedule_team.tid) 
