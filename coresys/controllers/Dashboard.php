...
class Dashboard extends MY_Controller {
	...
	public function index() {
		...
		$this->data['summary_kelolaan'] = $this->total_kelolaan();
	...
	public function summary() {
		...
		// PAGI
		$result['total_job_pagi'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."'
		")->row()->cnt;
		
		$result['job_pagi_done'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND 
			(
				trans_clean_detail.action_pagi!='' AND 
				trans_clean_detail.action_pagi!='PENDING' AND 
				trans_clean_detail.action_pagi!='CLOSED'
			)
		")->row()->cnt;
		
		$result['job_pagi_pending'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND trans_clean_detail.action_pagi='PENDING'
		")->row()->cnt;
		
		$result['job_pagi_complaint'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_complaint LEFT JOIN trans_complaint_detail ON(trans_complaint_detail.id_detail=trans_complaint.id)
			WHERE trans_complaint.date='".date("Y-m-d")."' AND trans_complaint_detail.action_pagi!=''
		")->row()->cnt;
		
		$result['job_pagi_closed'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND trans_clean_detail.action_pagi='CLOSED'
		")->row()->cnt;
		
		// SORE
		$result['total_job_sore'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."'
		")->row()->cnt;
		
		$result['job_sore_done'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND 
			(
				trans_clean_detail.action_sore!='' AND 
				trans_clean_detail.action_sore!='PENDING' AND 
				trans_clean_detail.action_sore!='CLOSED'
			)
		")->row()->cnt;
		
		$result['job_sore_pending'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND trans_clean_detail.action_sore='PENDING'
		")->row()->cnt;
		
		$result['job_sore_complaint'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_complaint LEFT JOIN trans_complaint_detail ON(trans_complaint_detail.id_detail=trans_complaint.id)
			WHERE trans_complaint.date='".date("Y-m-d")."' AND trans_complaint_detail.action_sore!=''
		")->row()->cnt;
		
		$result['job_sore_closed'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."' AND trans_clean_detail.action_sore='CLOSED'
		")->row()->cnt;
		
		return $result;
	}
	
	public function dashboard_val() {
		// PAGI
		$result['total_job_pagi'] = $this->db->query("
			SELECT count(*) as cnt FROM trans_clean LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
			WHERE trans_clean.date='".date("Y-m-d")."'
		...
		$kelolaan = $this->db->query(
			"SELECT kanwil, (SELECT count(*) FROM master_atm WHERE kanwil=master_kelolaan.kanwil) as count FROM master_kelolaan GROUP BY kanwil"
		);
		
		echo "<pre>";
		print_r($result);
		print_r($kelolaan->result());
	}
	
	public function total_kelolaan() {
		$kelolaan = $this->db->query(
			"
				SELECT 
				kanwil, 
				(SELECT count(*) FROM master_atm WHERE kanwil=master_kelolaan.kanwil) as count,
				(
					SELECT count(*) as cnt FROM trans_clean 
					LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
					LEFT JOIN master_kelolaan as mk ON (mk.id=trans_clean.id_lokasi)
					WHERE trans_clean.date='".date("Y-m-d")."' AND mk.kanwil=master_kelolaan.kanwil
				) as count_total_pagi,
				(
					SELECT count(*) as cnt FROM trans_clean 
					LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
					LEFT JOIN master_kelolaan as mk ON (mk.id=trans_clean.id_lokasi)
					WHERE 
					trans_clean.date='".date("Y-m-d")."' AND 
					mk.kanwil=master_kelolaan.kanwil AND 
					trans_clean_detail.action_pagi!=''
				) as count_done_pagi,
				(
					SELECT count(*) as cnt FROM trans_clean 
					LEFT JOIN trans_clean_detail ON(trans_clean_detail.id_detail=trans_clean.id)
					...
					trans_clean.date='".date("Y-m-d")."' AND 
					mk.kanwil=master_kelolaan.kanwil AND 
					trans_clean_detail.action_sore!=''
				) as count_done_sore
				FROM master_kelolaan GROUP BY kanwil
			"
		);
		
		return $kelolaan->result();
	}
