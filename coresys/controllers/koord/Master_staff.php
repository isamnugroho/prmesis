...
class Master_staff extends MY_Controller {
	...
	public function index() {
        ...
        return view('pages_koord/master_staff/index', $this->data);
	}
	...
	public function json() {
		...
		foreach($object as $rows) {
			...
			if($user($rows->nik)->num_rows()) {
				$add_acc = "<a onclick='editUserAccess(".json_encode($user($rows->nik)->row()).")' class='btn btn-primary btn-sm zoomsmall' style='background: linear-gradient(to bottom, #0072ff, #0072ff);border-radius: 4px;font-weight:bold;'><img style='float: left; margin: 1px 5px 0px 0px; height:15px; width:15px; ' src='".BASE_LAYOUT."/img/adddata.png'/>Edit Access</a>";	
			} else {
				$add_acc = "<a onclick='addUserAccess(".json_encode($rows).")' class='btn btn-primary btn-sm zoomsmall' style='background: linear-gradient(to bottom, #fe8c00, #f83600);border-radius: 4px;font-weight:bold;'><img style='float: left; margin: 1px 5px 0px 0px; height:15px; width:15px; ' src='".BASE_LAYOUT."/img/adddata.png'/>Add Access</a>";
			}
			
			$url = base_url().'/koord/master_staff';
			$list[$key]['no'] = $no;
			...
			$list[$key]['action'] = "
				<center>
				<a onclick='listModalPetugas(".json_encode($rows).")' class='btn btn-success btn-sm zoomsmall' style='background: linear-gradient(to bottom, #69c167, #13aa1d);border-radius: 4px;font-weight:bold;'><img style='float: left; margin: 1px 5px 0px 0px; height:15px; width:15px; ' src='".BASE_LAYOUT."/img/adddata.png'/>List Petugas</a>
				<a onclick='updateModal(".json_encode($rows).")' class='btn btn-warning btn-sm zoomsmall' style='background: linear-gradient(to bottom, #fe8c00, #f83600);border-radius: 4px;font-weight:bold;'><img style='float: left; margin: 1px 5px 0px 0px; height:15px; width:15px; ' src='".BASE_LAYOUT."/img/edit.png'/>Edit</a></center>";
	...
	public function json_list_petugas() {
		...
		foreach($object as $rows) {
			...
			$list[]= array(
				...
				<a onclick='deleteActionPetugas(\"".$url."/delete_petugas/".$rows->id."\")' class='btn btn-danger btn-sm zoomsmall' style='background: linear-gradient(to top, #ed213a, #93291e);border-radius: 4px;font-weight:bold;'><img style='float: left; margin: 1px 5px 0px 0px; height:18px; width:18px; ' src='".BASE_LAYOUT."/img/del.png'/>Delete</a></center>"
			);
		}
