...
class Detail_totjobpagi extends MY_Controller {
	var $data = array();
	...
	public function index() {
        return view('pages/detail_totjobpagi/index', $this->data);
	}
	...
	public function insert() {
		...
		if($id==null){
			// INSERT
			if (!$this->db->insert('detail_totjobpagi', $data)) {
				echo 'error';
			}
			echo 'success';
		} else {
			...
			// UPDATE
			$this->db->where('id', $id);
			if (!$this->db->update('detail_totjobpagi', $data)) {
				echo 'error';
			}
			echo 'success';
		}
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('detail_totjobpagi');
	...
	public function json() {
		...
		$query = "SELECT *, detail_totjobpagi.id as id, master_staff.nama as nama_staff, master_staff.alamat as alamat_staff, master_vendor.nama as nama_vendor, master_staff.id_vendor as vendor_id, master_staff.id as id_staff FROM detail_totjobpagi LEFT JOIN master_staff ON (detail_totjobpagi.username=master_staff.nik) LEFT JOIN master_vendor ON (master_staff.id_vendor=master_vendor.id)";
		...
		$team = function($team) {
			$array = $this->db->query("SELECT *, detail_totjobpagi.id as id, master_staff_petugas.nama as nama_staff, master_staff_petugas.alamat as alamat_staff, master_vendor.nama as nama_vendor, master_staff_petugas.id_vendor as vendor_id, master_staff_petugas.id as id_staff FROM detail_totjobpagi LEFT JOIN master_staff_petugas ON (detail_totjobpagi.username=master_staff_petugas.nik) LEFT JOIN master_vendor ON (master_staff_petugas.id_vendor=master_vendor.id) WHERE master_staff_petugas.id_koord = '$team'")->result_array();
		...
		foreach($object as $rows) {
			$no++;
			$url = base_url().'/detail_totjobpagi';
			$list[$key]['no'] = $no;
