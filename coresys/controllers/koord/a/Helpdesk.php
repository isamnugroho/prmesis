...
class Helpdesk extends MY_Controller {
	...
	public function index() {
        return view('pages/helpdesk/index', $this->data);
	}
	...
	public function insert() {
		...
		if($id==null){
			...
			// UPDATE
			$this->db->where('id', $id);
			if (!$this->db->update('helpdesk', $data)) {
				echo 'error';
			}
			echo 'success';
		}
	...
	public function delete($id) {
		$this->db->where('id', $id);
        $result = $this->db->delete('helpdesk');
	...
	public function json() {
		...
		$query = "SELECT *, helpdesk.id as id, master_staff.nama as nama_staff, master_staff.alamat as alamat_staff, master_vendor.nama as nama_vendor, master_staff.id_vendor as vendor_id, master_staff.id as id_staff FROM helpdesk LEFT JOIN master_staff ON (helpdesk.username=master_staff.nik) LEFT JOIN master_vendor ON (master_staff.id_vendor=master_vendor.id)";
		...
		$team = function($team) {
			$array = $this->db->query("SELECT *, helpdesk.id as id, master_staff_petugas.nama as nama_staff, master_staff_petugas.alamat as alamat_staff, master_vendor.nama as nama_vendor, master_staff_petugas.id_vendor as vendor_id, master_staff_petugas.id as id_staff FROM helpdesk LEFT JOIN master_staff_petugas ON (helpdesk.username=master_staff_petugas.nik) LEFT JOIN master_vendor ON (master_staff_petugas.id_vendor=master_vendor.id) WHERE master_staff_petugas.id_koord = '$team'")->result_array();
