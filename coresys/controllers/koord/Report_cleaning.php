...
class Report_cleaning extends MY_Controller {
	...
	public function index() {
        return view('pages_koord/report_cleaning/index', $this->data);
		// $date = (isset($_REQUEST['date_daily']) ? $_REQUEST['date_daily'] : "");
		
		
		// $date_montly = DateTime::createFromFormat('m/Y', "02/2022")->format('Y-m');
	...
	public function json() {
		...
		: "");
		$date_montly = (isset($_REQUEST['date_montly']) ? 
			DateTime::createFromFormat('m/Y', $_REQUEST['date_montly'])->format('Y-m')
		: "");
		
		// $date_range = (isset($_REQUEST['date_range']) ? 
			// DateTime::createFromFormat('m/Y', $_REQUEST['date_montly'])->format('Y-m')
		// : "");
		// $kanwil = (isset($_REQUEST['kanwil']) ? 
			// $_REQUEST['kanwil']
		...
		if($date_daily!=="") {
			...
			$extraWhere = "
							(`b`.`action_pagi`!='' OR `b`.`action_sore`!='') AND
							`b`.`jenis_job`!='switch' AND
							`a`.`pic`='".$this->data['user_access']['username']."'
			";
		}
		$groupBy = "";
		$having = "";
	...
	public function excel_daily() {
		...
		$filename = 'report_cleaning_'.date("Y_m_d");
